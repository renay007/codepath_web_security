...
// Cipher method to use for symmetric encryption
const CHECKSUM_METHOD = 'MD5';

function create_checksum($data, $checksum=CHECKSUM_METHOD) {
	switch ($checksum) {
		case "MD5":
			return md5($data);
			break;
...
function verify_checksum($data, $signature, $checksum) {
	return $signature==create_checksum($data,$checksum);
}
