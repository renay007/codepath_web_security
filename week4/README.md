...
* [x]  Bonus Objective 1: Identify security flaw in Objective #4 (requiring login on staff pages)
  * [x]  Identify the security principal not being followed.
	* Session hijacking - User is not logging in over SSL. Therefore the session ID is sent back to the server without being encrypted.
  * [x]  Write a short description of how the code could be modified to be more secure.
	* Code could be modified to make sure all requests (access-restricted pages) are made over SSL.
	* We also need to change cookies configurations to be "secure" with session.cookie_secure = 1
...
<img src='http://i.imgur.com/GAgHHMF.gif' title='Video Walkthrough' width='' alt='Video Walkthrough' />
